...
def get_movie_url(website_url):
    try:
        ...
        #获取网页数据
        request_url_data = urllib.request.urlopen(request_url, timeout=3).read()
        #转换网页数据字符串
    ...
    for movie_link, movie_name in re_url_list:
        #生成完整网页连接
        movie_link = web_site + movie_link
        #添加请求头
        ...
        #正则匹配下载链接+
        # re_movie_link = re.search(r'(magnet:\?xt=urn:btih|ftp://d|ftp://j|ftp://z|ftp://2|ftp://y|ftp://q|btbo://|ftp://a):(.*?).(mp4|mkv|rm|rmvb|avi|wmv)', movie_link_url_text)
        re_movie_link = re.search(r'(magnet:\?xt=urn:btih|ftp://\w|btbo://):(.*?).(mp4|mkv|rm|rmvb|avi|wmv)', movie_link_url_text)
        #下载链接拼接
        # print(re_movie_link)
        ...
        movie_download = re_movie_link.group(1) + ':' + re_movie_link.group(2) + '.' + re_movie_link.group(3)
